...
func TestCreateProduct(t *testing.T) {
	...
	tcs := []struct {
		...
		},
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				mock.ExpectExec("INSERT INTO products (name, image, category, description, rating, num_reviews, price, count_in_stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").WillReturnResult(sqlmock.NewErrorResult(fmt.Errorf("error getting the last insert id")))
	...
	tcs := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				...
				// Initialize order rows to test and add contents based on the "o" struct that has been declared before
				...
				// Ensuring the query will return orows that has been initialized before
				...
				// Trying to execute the GetOrder function and it should not return an error and sets its id to 1
				mo, err := st.GetOrder(context.Background(), 1)
				require.NoError(t, err)
				require.Equal(t, int64(1), mo.ID)

				// Ensuring the items property aligns with each items in the loop
