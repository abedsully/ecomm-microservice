...
func (ms *MySQLStorer) UpdateProduct(ctx context.Context, p *Product) (*Product, error) {
	_, err := ms.db.NamedExecContext(ctx, "UPDATE products SET name=:name, image:=image, category:=category, description=:description, rating:=rating, num_reviews:=num_reviews, price:=price, count_in_stock:=count_in_stock",  p)
...
func (ms *MySQLStorer) CreateOrder(ctx context.Context, o *Order) (*Order, error) {
	err := ms.execTx(ctx, func(tx *sqlx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("error creating orer: %w", err)
		}
...
func (ms *MySQLStorer) GetOrder(ctx context.Context, id int64) (*Order, error) {
	...
	// & is used as a reference to orders variable
	...
	// := is not used because err has been declared earlier
...
func (ms *MySQLStorer) ListOrders(ctx context.Context) ([]Order, error) {
	...
	for i := range orders {
		var items []OrderItem
		err = ms.db.GetContext(ctx, &items, "SELECT * FROM order_items WHERE order_id=?", orders[i].ID)
